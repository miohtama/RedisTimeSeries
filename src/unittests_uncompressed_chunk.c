...
MU_TEST(test_Uncompressed_NewChunk) {
    ...
    for (size_t chunk_size = 2; chunk_size < max_chunk_size; chunk_size += 64) {
        ...
        mu_assert_short_eq(0, Uncompressed_NumOfSample(chunk));
...
MU_TEST(test_Uncompressed_Uncompressed_AddSample) {
    ...
    while (rv != CR_END) {
        ...
        if (rv != CR_END) {
            total_added_samples++;
            mu_assert_short_eq(total_added_samples, Uncompressed_NumOfSample(chunk));
        }
    }
...
MU_TEST(test_Uncompressed_Uncompressed_UpsertSample) {
    ...
    mu_assert_int_eq(total_added_samples, Uncompressed_NumOfSample(chunk));
    Uncompressed_FreeChunk(chunk);
}
...
MU_TEST(test_Uncompressed_Uncompressed_UpsertSample_DuplicatePolicy) {
    ...
    Chunk *chunk = Uncompressed_NewChunk(chunk_size);
    mu_assert(chunk != NULL, "create uncompressed chunk");
    mu_assert_int_eq(0, Uncompressed_NumOfSample(chunk));
    ChunkResult rv = CR_OK;
    ...
    mu_assert_int_eq(1, Uncompressed_NumOfSample(chunk));
    const u_int64_t firstTs = Uncompressed_GetFirstTimestamp(chunk);
    mu_assert_int_eq(1, firstTs);
    Uncompressed_GetSampleValueAtPos(chunk, 0, &value);
    mu_assert_double_eq(-0.5, value);
    ...
    Uncompressed_GetSampleValueAtPos(chunk, 0, &value);
    mu_assert_double_eq(-0.2, value);
    Uncompressed_FreeChunk(chunk);
}

MU_TEST(test_Uncompressed_Uncompressed_UpsertSample_DuplicatePolicy_Sum) {
    const size_t chunk_size = 100; // 100 bytes (data) chunck
    Chunk *chunk = Uncompressed_NewChunk(chunk_size);
    mu_assert(chunk != NULL, "create uncompressed chunk");
    mu_assert_int_eq(0, Uncompressed_NumOfSample(chunk));
    ChunkResult rv = CR_OK;
    Sample s1 = { .timestamp = 1, .value = 1.0 };
    rv = Uncompressed_AddSample(chunk, &s1);
    mu_assert(rv == CR_OK, "add sample");
    ...
    int total_sum = 99;
    double value;
    double outsideSum = 1.0;
    for (size_t i = 0; i < total_sum; i++) {
        UpsertCtx uCtx = {
            .inChunk = chunk,
            .sample = { .timestamp = 1, .value = 1.0 },
        };
        // DP_SUM should sum both values
        Uncompressed_GetSampleValueAtPos(chunk, 0, &value);
        mu_assert_double_eq(outsideSum, value);
        rv = Uncompressed_UpsertSample(&uCtx, &size, DP_SUM);
        mu_assert(rv == CR_OK, "duplicate sum");
        mu_assert_int_eq(1, Uncompressed_NumOfSample(chunk));
        const u_int64_t firstTs = Uncompressed_GetFirstTimestamp(chunk);
        mu_assert_int_eq(1, firstTs);
        Uncompressed_GetSampleValueAtPos(chunk, 0, &value);
...
MU_TEST_SUITE(uncompressed_chunk_test_suite) {
    ...
    MU_RUN_TEST(test_Uncompressed_Uncompressed_UpsertSample_DuplicatePolicy_Sum);
}
